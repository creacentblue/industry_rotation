...
end_date = datetime.datetime.strptime("2016-07-01", "%Y-%m-%d")
factor = "QF_NetProfit"
cmd = ' '.join((
    "select TRADINGDATE, SYMBOL, FACTORVALUE",
    "FROM dbo.{}".format(factor),
...
NPFData = get_last(NPFData)  # 10-18
...
# 聚集函数,按行业聚集
def sort_and_avg(data, result):
    sort_list = sorted(data, key=lambda e: e.__getitem__('INDUSTRY'), reverse=True)
...
while True:
    if i > len(NPData)-1:
        break
    if NPData[i]['TRADINGDATE'] == pre_date:
        temp_list.append(NPData[i])
        i += 1
    else:
        pre_date = NPData[i]['TRADINGDATE']
        re = []
        NPData_ += sort_and_avg(temp_list, re)
        temp_list = []
re = []
NPData_ += sort_and_avg(temp_list, re)
# 求出平均值
pre_date = NPFData[0]['TRADINGDATE']
i = 0
temp_list = []
NPFData_ = []
while True:
    if i > len(NPFData)-1:
        break
    if NPFData[i]['TRADINGDATE']==pre_date:
        temp_list.append(NPFData[i])
        i += 1
    else:
        pre_date = NPFData[i]['TRADINGDATE']
        re = []
        NPFData_ += sort_and_avg(temp_list, re)
        temp_list = []
re = []
NPFData_ += sort_and_avg(temp_list, re)
# 求出平均值
pre_date = PEFData[0]['TRADINGDATE']
i = 0
temp_list = []
PEFData_ = []
while True:
    if i > len(PEFData)-1:
        break
    if PEFData[i]['TRADINGDATE']==pre_date:
        temp_list.append(PEFData[i])
        i += 1
    else:
        pre_date = PEFData[i]['TRADINGDATE']
        re = []
        PEFData_ += sort_and_avg(temp_list, re)
        temp_list = []
re = []
PEFData_ += sort_and_avg(temp_list, re)
...
for item in PEFData_:  # 清洗数据
    ...
    # 若匹配成功则进行计算
    if ~flag:
        if (NPFData_[j]['VALUE_AVG']/NPData_[k]['VALUE_AVG']) >= 0:
            PEG = item['VALUE_AVG']/Decimal.from_float(
                ((math.sqrt(NPFData_[j]['VALUE_AVG']/NPData_[k]['VALUE_AVG'])-1)*100))
        else:
            PEG = item['VALUE_AVG'] / Decimal.from_float(
                ((-math.sqrt(-NPFData_[j]['VALUE_AVG'] / NPData_[k]['VALUE_AVG']) - 1) * 100))
